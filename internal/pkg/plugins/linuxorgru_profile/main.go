...
func NewPlugin() *LinuxOrgRuProfilePlugin {
	return &LinuxOrgRuProfilePlugin{fetcher: deeperhttp.NewClient(config.LoadConfig())}
}

func (p *LinuxOrgRuProfilePlugin) Register() error {
...
func (p *LinuxOrgRuProfilePlugin) FollowTrace(trace entities.Trace) ([]entities.Trace, error) {
	if !p.Matches(trace) {
		return nil, nil
	}
