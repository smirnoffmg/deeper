...
func NewPlugin() *LaunchpadProfilePlugin {
	return &LaunchpadProfilePlugin{fetcher: deeperhttp.NewClient(config.LoadConfig())}
}

func (p *LaunchpadProfilePlugin) Register() error {
...
func (p *LaunchpadProfilePlugin) FollowTrace(trace entities.Trace) ([]entities.Trace, error) {
	if !p.Matches(trace) {
		return nil, nil
	}
